...
Java_uk_lgl_modmenu_FloatingModMenuService_getFeatureList(JNIEnv *env, jobject context) {
    ...
    MakeToast(env, context, OBFUSCATE("Mod criado por FREEZA MODS"), Toast::LENGTH_LONG); // <---MUDE ESSE NOME PARA O SEU NOME OU O NOME DO SEU CANAL.

    const char *features[] = {
            OBFUSCATE("Category_Funções do Mod"), //Not counted
            OBFUSCATE("Toggle_função 1"),
            OBFUSCATE("Toggle_função 2"),
